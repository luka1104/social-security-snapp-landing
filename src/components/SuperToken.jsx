...
async function upgradeTokens(amount) {
  ...
  const daixAddress = "0x88271d333C72e51516B67f5567c728E702b3eeE8"
  const daix = new SuperToken(sf, daixAddress)
...
async function downgradeTokens(amount) {
  ...
  const chainId = await window.ethereum.request({ method: "eth_chainId" })
  const sf = await new Framework.create({
